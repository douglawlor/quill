...
| ID | Item | Area | Size | Status | Acceptance criteria |
| --- | --- | --- | --- | --- | --- |
| CQ-1 | Decompose main_frame into cohesive modules | Code quality | XL | In progress | main_frame is split into controllers or mixins with no behavior change; characterization tests pass before and after. **First seam delivered:** the 25-method browse-mode navigation cluster (~455 lines) was extracted verbatim from `main_frame.py` into `quill/ui/main_frame_browse.py` as `BrowseModeMixin`, which `MainFrame` now inherits; calls resolve identically through the MRO so behavior is unchanged. **Second seam delivered:** the four selection-scoped AI writing actions plus their three private helpers were extracted into `quill/ui/main_frame_ai_actions.py` as `AiActionsMixin` (now in the `MainFrame` MRO); because these are public methods, the GATE-6 public-surface snapshot was regenerated via `python -m quill.tools.ui_surface --write` along the sanctioned collaborator-move path, and the runtime surface is unchanged via inheritance. In the same pass two oversized `quill/core` modules were decomposed to satisfy GATE-11 by extraction (never by raising a budget): `settings_registry.py` (984→158 lines) moved its `SettingSpec`/`SettingGroup`/`SETTING_*` data tables into a new `settings_specs.py` (re-exported), and `settings.py` (642→570 lines) moved `STATUS_BAR_ITEMS` and the status-bar value normalizers into a new `settings_normalizers.py` (re-exported). `main_frame.py` is now 22,866 lines and every touched GATE-11 budget ratcheted down with the change; all moves are proven behavior-identical by the main_frame UI suite, the regenerated characterization fixture, and the core settings/registry suites. **Third seam delivered:** the contiguous 17-method status-bar cluster (status-bar item resolution, value/label/help text, cell construction and refresh, keyboard navigation, activation, hide/restore, and the per-cell context menu, ~400 lines) was extracted verbatim from `main_frame.py` into `quill/ui/main_frame_statusbar.py` as `StatusBarMixin`, taking the `_StatusBarCell` dataclass with it (re-imported for the annotation that stays behind); because every moved method is private, the GATE-6 public-surface snapshot was unchanged and needed no regen. `main_frame.py` dropped 22,866→22,460 lines (budget ratcheted to 22,460); the new module is 433 lines (under the 600 cap). Verified live on Windows: the full 322-test UI suite, GATE-11, GATE-6 characterization, the dialog-contract/inventory gates, and the A11Y-4 banned-pattern gate all pass. **Fourth seam delivered:** the contiguous 6-method intellisense/word-prediction popup cluster (`_show_intellisense_popup`, `_refresh_intellisense_popup`, `_hide_intellisense_popup`, `_dismiss_intellisense_popup`, `_apply_intellisense_selection`, `_handle_intellisense_key_down`, ~130 lines) was extracted verbatim from `main_frame.py` into `quill/ui/main_frame_intellisense.py` as `IntellisensePopupMixin` (now in the `MainFrame` MRO); the two `quill.core.intellisense` helpers it needs (`build_intellisense_suggestions`, `collect_document_words`) moved with it and ruff trimmed those imports from `main_frame.py`. Because every moved method is private, the GATE-6 public-surface snapshot was unchanged and needed no regen. `main_frame.py` dropped 22,460→22,332 lines (budget ratcheted to 22,332); the new module is under the 600 cap. Verified live on Windows: the full 322-test UI suite, GATE-11, GATE-6 characterization, the dialog-contract/inventory gates, and the A11Y-4 banned-pattern gate all pass. The mixin pattern is now established for the remaining clusters (menu construction, QUILL key/Quick Nav, selection/marks, file/session lifecycle), so this stays honestly In progress (XL) until those land. |
...
| DLG-3 | Full dialog estate review and conversion to native (or enhanced-native) wxPython constructs | Code quality | XL | In progress | The flagship Tier 4 refactor, done first. Folds in and supersedes issue #73 (and #72). The full plan is canonical in the PRD §9.13 (Dialog estate governance), and per-phase progress is tracked in the DLG-3.0–DLG-3.8 sub-rows above. **Delivered (enforcement engine, Phase 0 + Phase 1):** the authoritative dialog estate is now generated from source, not a checklist — `quill/tools/dialog_inventory.py` AST-scans all of `quill/**/*.py` and records every dialog *surface* (each `wx.Dialog(...)`, every stock wx dialog, and every `show_web_form(...)` call — **154** surfaces today: 100 `native`, 49 `hardened_custom`, 5 `web`) under a stable, line-independent key (`<module>::<enclosing_qualname>::<kind>`) with its sanctioned classification. The committed registry snapshot `tests/unit/ui/fixtures/dialog_inventory.json` is enforced by two gates: `tests/unit/ui/test_dialog_inventory.py` (fails on any new/moved/removed/reclassified dialog or unsanctioned surface) and a new registry cross-check inside the GATE-4 A11Y-4 banned-pattern gate (`_check_dialog_registry` in `quill/tools/check_banned_patterns.py`, run in Security CI) that fails the build on any unregistered or misclassified dialog. Adding a dialog now *forces* it into the registry via a deliberate `python -m quill.tools.dialog_inventory --write`, whose classification is reviewed in the diff — the "magical" gating now documented in PRD §9.13. `.github/copilot-instructions.md` gains a "Dialog Excellence Mandates" block + Dialog Change Checklist encoding this. **Remaining (honest, PRD §9.13 Phases 6 + 8):** Phases 2–5 and 7 are now Done (native-conversion triage, enhanced-native standardization, web-surface governance guard, startup/onboarding hardening guard, and CQ-16 dialog-launch characterization — see the DLG-3.2–DLG-3.5 and DLG-3.7 sub-rows). What remains is the assistant/AI tool dialog consolidation (Phase 6, folds in DLG-2) and the manual NVDA/JAWS/Narrator SR pass across `dialogs.md` (Phase 8), which requires a live Windows screen-reader runtime; this item stays In progress until those land. Every single dialog across `quill/ui/*` is reviewed, and any custom-drawn or hand-rolled dialog that *could* conform to a standard surface is converted to one of **two** sanctioned kinds: **native** (`wx.MessageDialog`/`wx.MessageBox` for confirms, saves, and simple messages) or the existing **accessible web surfaces** (`AccessibleChatView` for input, `AccessibleHtmlDialog` for display, `MarkdownPreviewDialog` for previews) for anything that displays rich content or captures input — reusing what already ships, no new component. The motivation is risk: the ~15 hand-rolled dialog classes and 39 raw `wx.Dialog(...)` usages across 8 UI files are where controls fail to work, focus lands on the wrong control (e.g. sticky-note editor landing on Save instead of the text field), and rendering breaks — none of which a Linux/source-contract bar catches — and they include at least one first-run onboarding **startup crash** on Windows and macOS with a screen reader active. Scope: (1) inventory all dialogs (forms, interactive tools, pickers, wizards, nested and startup) and classify each as keep-as-native, convert-to-native, convert-to-web, or genuinely-must-stay-custom with a written reason; (2) convert every convertible dialog (sticky notes, the `assistant_tools.py` tool dialogs, `AIModelDialog`, `TrainStyleDialog`, onboarding/welcome which must render rather than dump raw HTML per #72, and the confirm/consent prompts in `main_frame.py`) preserving behavior and fixing focus/keyboard handling, which removes the onboarding crasher; (3) **strengthen the A11Y-4 dialog-contract guard** so the residual-custom-dialog bug class (controls not wired, no default, focus not returned, parent-ownership/sizer mismatches, rendering traps) is caught at author time and cannot regress. Supersedes the batch ambition of DLG-2 (DLG-2's individual AI-tool conversions fold under this) and is the largest single refactor in the 1.0 plan — sequenced **before** CQ-1/CQ-16 decomposition so the dialog estate is sound before the monolith is split. Acceptance: no hand-rolled `wx.Dialog` subclasses remain for confirm/message flows; input/display dialogs use the existing accessible web surfaces; first-run onboarding completes without crashing on Windows and macOS with a screen reader on. Gated by the strengthened A11Y-4 guard, `dialogs.md`, and screen-reader verification on Windows. |
| DLG-3.0 | Phase 0 — authoritative source-generated dialog registry + gates | Code quality | M | Done | `quill/tools/dialog_inventory.py` AST-scans all of `quill/**/*.py` and records every dialog surface (154: 100 `native`, 49 `hardened_custom`, 5 `web`) under a stable key with its sanctioned classification; snapshot `tests/unit/ui/fixtures/dialog_inventory.json` enforced by `tests/unit/ui/test_dialog_inventory.py` and the A11Y-4 banned-pattern cross-check. |
| DLG-3.1 | Phase 1 — strengthened A11Y-4 static dialog-contract guard | Code quality | M | Done | `_check_dialog_registry` in `quill/tools/check_banned_patterns.py` fails the build on any unregistered or misclassified dialog; the existing `wx.ALIGN_RIGHT` and raw-`wx.Dialog` destroy-path checks remain. `.github/copilot-instructions.md` carries the Dialog Excellence Mandates block + Dialog Change Checklist. |
| DLG-3.T | Triage — classify all 49 `hardened_custom` dialogs into convert/harden/keep buckets | Code quality | S | Done | Every `hardened_custom` surface received a written disposition via a live wx 4.2.5 source/AST audit on this Windows machine. **Result: 0 convert, 48 harden, 1 keep-web** (`AskQuillChatDialog`). A source scan for `EVT_PAINT`/`wx.lib.agw`/`OnPaint`/`wx.PaintDC`/owner-draw/`wx.html2` found **zero** custom-drawn dialogs — every `hardened_custom` surface is already a stock-widget `wx.Dialog` container (ListBox, TextCtrl, SearchCtrl, TreeCtrl, multi-button workflows), so "convert to native" is, in the literal wx sense, already true. The real work is contract hardening, not rewrites. |
| DLG-3.2 | Phase 2 — native conversion wave (hand-rolled single confirm/choice/text → stock one-shot) | Code quality | L | Done (no applicable work) | The triage (DLG-3.T) found **zero** lossless one-shot conversion candidates: every `hardened_custom` dialog has genuine multi-control complexity (live search, lists, preview panes, tree controls, multi-action button rows) and would lose capability if flattened into a single `wx.MessageDialog`/`wx.SingleChoiceDialog`/`wx.TextEntryDialog`. Read-only instructional displays correctly use multiline `wx.TextCtrl` per the repo convention, not transient message boxes. Honest outcome: no dialog warrants flattening, so this wave converts nothing. |
| DLG-3.3 | Phase 3 — enhanced-native standardization wave (multi-control dialogs onto one contract) | Code quality | L | Done | All **49** `hardened_custom` dialogs now converge on one focus/default/lifecycle grammar via the shared `dialog_contract` helpers. A machine-derived AST audit found only 5 surfaces off-contract; 4 were wired onto `apply_modal_ids` (+ routed direct `ShowModal()` through the announcing `_show_modal_dialog`, + deterministic initial focus): `_present_quill_key_help`, `_offer_crash_recovery`, `_present_quick_nav`, `_choose_searchable_option` (the 5th, `show_watch_folder_status`, is a correctly-hardened modeless monitor). A durable guard `tests/unit/ui/test_dialog_hardening_contract.py` now AST-asserts every `hardened_custom` surface wires the contract, blocking future drift. None flattened — live search, lists, and previews preserved. |
| DLG-3.4 | Phase 4 — web-surface standardization (only where rich rendering/forms justified) | Code quality | M | Done | A live source audit confirmed the rich-render estate is already clean: all raw HTML rendering primitives (`wx.html.HtmlWindow`/`.SetPage`) are confined to the single sanctioned `preview_dialog.py` ImportError fallback; every other rich surface routes through the `wx_accessible_webview` adapters (`preview_dialog.py`, `accessible_webview.py`) or the `assistant_panel.py` chat dialog, each with a native fallback (list box + text field for chat; `wx.html` for previews). Onboarding/welcome tabs carry **Markdown**, never HTML (`build_welcome_guide` emits Markdown; the Startup Wizard renders through the `MarkdownPreviewDialog` web surface, not a document tab). New guard `tests/unit/ui/test_web_surface_governance.py` (4 tests) locks all of this in: it fails if a raw HTML primitive escapes `preview_dialog.py`, if either sanctioned web surface loses its native fallback, or if the welcome-guide tab content becomes HTML. |
| DLG-3.5 | Phase 5 — startup/onboarding hardening (wizard, first-run, trust consent, crash recovery) | Code quality | M | Done | The two code-level acceptance claims are implemented and now guarded. The screen-reader **startup-crash path is retired**: `_run_deferred_startup_tasks` isolates every deferred step (IPC poll, SR detection, trust consent, crash recovery, first-run onboarding, watch folders, update check) in its own `try/except`, persists the traceback to `logs/startup-errors.log` via `_report_startup_task_failure`, and keeps Quill open. **Explicit consent is preserved**: `_show_trust_consent_onboarding` marks consent complete only on an explicit accept; a decline never records consent and, during startup, closes the app instead of silently continuing. New guard `tests/unit/ui/test_startup_onboarding_hardening.py` (4 tests) characterizes step-isolation, the accept/decline consent contract, and the declined-startup close path. Deterministic focus across the chained startup modals is structurally enforced by the DLG-3.3 hardening contract and confirmed live in DLG-3.8. |
| DLG-3.6 | Phase 6 — assistant/AI tool dialog consolidation (folds in DLG-2) | Code quality | L | Todo | `assistant_tools.py`, `ai_model_panel.py`, `style_panel.py`, `assistant_panel.py` dialogs meet the same modal/focus/error contract with safe async/"busy" semantics, converted individually and screen-reader-verified (the DLG-2 work folded here). |
| DLG-3.7 | Phase 7 — CQ-16 characterization expansion around dialog-launch command paths | Code quality | M | Done | New characterization suite `tests/unit/ui/test_dialog_launch_characterization.py` (4 tests) pins the return value and observable side effects of the public guide/preview launch commands (`open_welcome_guide`, `open_user_guide` missing-file fallback, `open_third_party_notices`, `show_startup_wizard_page`): the surface opened, the location-ring reset, the title refresh, and the exact status line. These regression tests protect the command surface before any CQ-1 decomposition reshapes `main_frame.py`. |
| DLG-3.8 | Phase 8 — manual NVDA/JAWS/Narrator SR pass across `dialogs.md` | Code quality | M | Todo | NVDA baseline, JAWS spot (startup, assistant, sticky notes, watch profiles), Narrator sanity; each `dialogs.md` row carries pass/fail evidence. Requires a live Windows screen-reader runtime. |
...
| CQ-21 | Guard TOML parsing in compliance | Code quality | S | Done | `quill/core/compliance.py` wraps `tomllib` parsing in `_load_pyproject`, which catches `tomllib.TOMLDecodeError` and re-raises a dedicated `ComplianceConfigError` whose message names the offending `pyproject.toml` path (a missing file still raises `FileNotFoundError` so absence stays distinguishable from corruption). `test_corrupt_pyproject_raises_clear_error` in `tests/unit/core/test_compliance.py` writes a syntactically invalid TOML file and asserts both `dependency_names_from_pyproject` and `dependency_requirements_from_pyproject` raise `ComplianceConfigError` with the path in the message — so a corrupted file produces a clear, located error instead of a silent abort or a bare decode traceback. |
