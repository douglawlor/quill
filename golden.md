...
| FLAG-1 | Feature-flag respect is a UI contract | Foundations | M | Todo | Disabling any feature through `FeatureManager` consistently hides or disables every surface it owns: commands and the palette (already filtered by `feature_id`), menu items, status-bar cells, QUILL key and Quick Nav entries, settings groups, and first-run/onboarding prompts. No command is left orphaned or dead when its feature is off, and a feature with unmet dependencies stays off. A contract test enumerates `FEATURE_DEFINITIONS` and asserts each owned surface is gated; representative features (watch, dictation, macros, AI) are covered. |
| FLAG-2 | Every new feature ships a FeatureDefinition | Foundations | S | Todo | A definition-of-done item and a check require each user-facing feature to register a `FeatureDefinition` (id, name, description, dependencies, maturity, privacy, off-by-default where appropriate) and to wire its commands and surfaces to that id. A test fails when a feature-bearing command references a `feature_id` absent from `FEATURE_DEFINITIONS`. |
...
| ID | Item | Area | Size | Status | Acceptance criteria |
| --- | --- | --- | --- | --- | --- |
| GLOW-1 | Adopt quill-glow-core as the shared engine | Suite | M | Todo | Step 1 (prerequisite): diagnose and fix the failing CI/tests in the `s:\code\glow` repo and bring its suite green so QUILL builds on a stable engine; commit those fixes in the `glow` repo. Step 2: QUILL audits and fixes via `quill-glow-core` (`s:\code\quill-glow-core`) with the GLOW backend (`from_glow_backend` -> `acb_large_print_core`) when present and the safe no-op fallback when absent; in-editor reports unchanged for users; tests cover both paths. Coordinate changes across the three repos and keep the shared API stable. Ignore the stale `s:\code\glow-7.0.0` snapshot. |
...
| --- | --- | --- | --- | --- | --- |
| Tier 1 | Protect users and unlock the team | 23 | 21 | 2 | FLAG-1, FLAG-2 |
...
| Tier 3 | GLOW accessibility engine | 6 | 0 | 6 | GLOW-1..5, WATCH-8 |
...
| Tier 6 | BITS Whisperer and expansion | 27 | 0 | 27 | BW-1..10, WATCH-9, NAV-10, AI-11, AI-12, AI-18, FEAT-12..18, LINUX-1, LINUX-2, ECO-1, L10N-1, COLLAB-1 |
| **Total** | All tiers | **156** | **21** | **135** | |
